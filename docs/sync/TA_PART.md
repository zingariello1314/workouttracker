...
| `AUTH_JWT_SECRET` | **Fortement recommandé** (backend) | À mettre dans **`backend/.env`** ou **`.env` racine** (le serveur charge les deux). Même valeur à chaque redémarrage : sinon un secret aléatoire par processus invalide les access JWT jusqu’au prochain `/auth/refresh` ; les refresh tokens restent dans `backend/auth_server.db`. Voir `backend/.env.example`. |
