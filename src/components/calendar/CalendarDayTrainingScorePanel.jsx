...
import { Dumbbell, Sparkles, CircleDashed } from 'lucide-react';
...
function AbsentCriterionRow({ label, detail }) {
  return (
    <div className="rounded-lg border border-dashed border-slate-700/60 bg-slate-950/30 p-3 opacity-80">
      <div className="mb-1 flex items-center gap-2 text-sm text-slate-500">
        <CircleDashed className="h-3.5 w-3.5 shrink-0" aria-hidden />
        <span>{label}</span>
        <span className="ml-auto text-xs uppercase tracking-wide text-slate-600">non compté</span>
      </div>
      {detail ? <p className="text-xs leading-relaxed text-slate-500">{detail}</p> : null}
    </div>
  );
}
 ...
 */
export default function CalendarDayTrainingScorePanel({ strength, holistic, holisticDetailId, t }) {
  const tr = t || ((k, d) => d);
  ...
  return (
    <div className="space-y-4">
      ...
      {holistic?.score != null ? (
        <div
          id={holisticDetailId || 'calendar-day-holistic-detail'}
          className="scroll-mt-6 rounded-xl border border-emerald-500/35 bg-emerald-950/15 p-4"
        >
          <div className="mb-3 flex items-center justify-between gap-3">
          ...
          <p className="mt-2 text-xs text-emerald-200/75">
            {tr(
              ...
              'Combine musculation, endurance, Garmin, étirements, repas et pesée si saisis. Plus vous complétez, plus la note est fiable — sans pénaliser l’absence de sommeil.'
            )}
          </p>
          <div className="mt-3 space-y-2">
            ...
            {holistic.absentCriteria?.map((c) => (
              <AbsentCriterionRow key={c.id} label={c.label} detail={c.detail} />
            ))}
          </div>
        </div>
      ) : null}
