...
const DashboardBooksModule = () => {
  ...
  return (
    <div className="relative overflow-hidden rounded-3xl border border-violet-500/30 bg-gradient-to-br from-slate-950/90 via-indigo-950/80 to-violet-950/70 shadow-[0_0_80px_rgba(124,58,237,0.25)]">
      <div
        className="pointer-events-none absolute inset-0 opacity-60"
        style={{
          background:
            'radial-gradient(circle at 10% 10%, rgba(168,85,247,0.25), transparent 50%), radial-gradient(circle at 90% 20%, rgba(59,130,246,0.22), transparent 52%), radial-gradient(circle at 65% 90%, rgba(236,72,153,0.16), transparent 45%)',
          mixBlendMode: 'screen'
        }}
      />
      <div className="relative p-6 md:p-7 lg:p-8 space-y-5">
        <div className="flex items-center justify-between gap-3">
          <div className="flex items-center gap-3">
            <div className="rounded-2xl bg-violet-500/20 border border-violet-300/40 p-2">
          ...
          <button
            ...
            className="h-8 px-3 rounded-lg border border-violet-400/40 bg-violet-500/20 text-violet-100 text-xs font-medium hover:bg-violet-500/30"
          >
            Ouvrir Livres
          </button>
        </div>

        <div className="rounded-2xl border border-violet-400/20 bg-slate-950/40 overflow-hidden">
          <Suspense
            fallback={
              <div className="h-[56vh] flex items-center justify-center text-slate-300 text-sm">
        ...
        {!isLoading && (
          <div className="space-y-3">
            <div className="flex items-center justify-between gap-2">
              <div className="text-sm font-semibold text-violet-100">Bibliothèque par statut</div>
              <button
                type="button"
                onClick={openAddBook}
                className="h-7 px-2.5 rounded-md border border-emerald-400/40 bg-emerald-500/15 text-emerald-100 text-[11px] font-medium hover:bg-emerald-500/25"
              >
            ...
            {[
              {
                ...
                badge: 'En cours',
                badgeCls: 'bg-blue-500/20 text-blue-200 border-blue-400/30',
              ...
              },
              {
                ...
                badge: 'Terminé',
                badgeCls: 'bg-emerald-500/20 text-emerald-200 border-emerald-400/30',
              ...
              },
              {
                ...
                badge: 'À lire',
                badgeCls: 'bg-violet-500/20 text-violet-200 border-violet-400/30',
                list: byStatusRows.toRead
              }
            ].map((row) => (
              <div key={row.key} className="rounded-xl border border-violet-400/20 bg-slate-900/40 p-3">
                <div className="flex items-center justify-between mb-2">
                  <div className="text-sm font-semibold text-slate-100">{row.title}</div>
                  <button
                    type="button"
                    onClick={openBooksTab}
                    className="text-[11px] px-2 py-1 rounded-md border border-violet-400/35 text-violet-100 hover:bg-violet-500/20"
                  >
                    Voir tout
                  </button>
                </div>
                {row.list.length === 0 ? (
                  <div className="text-xs text-slate-400">Aucun livre dans cette catégorie.</div>
                ) : (
                    ...
                    {row.list.slice(0, 6).map((book) => {
                      ...
                      return (
                        <button
                          ...
                          className="w-full text-left rounded-lg border border-slate-700/70 bg-slate-950/55 hover:bg-slate-900/80 transition-colors p-2.5 flex items-center gap-3"
                        >
                          <div className="h-14 w-11 rounded overflow-hidden border border-slate-700/70 bg-slate-900/80 shrink-0">
                            {bookCover ? (
                              <img src={bookCover} alt={book.title || 'Livre'} className="h-full w-full object-cover" />
                            ) : (
                              <div className="h-full w-full flex items-center justify-center text-[10px] text-slate-500">No cover</div>
                            )}
                          </div>
                          <div className="min-w-0 flex-1">
                            <div className="text-sm font-semibold text-white truncate">{book.title || 'Livre sans titre'}</div>
                            <div className="text-xs text-slate-400 truncate">{book.author || 'Auteur non renseigné'}</div>
                            <div className="mt-1 flex items-center gap-2 text-[11px]">
                              ...
                              <span className="text-slate-300">{book.pages || 0} p</span>
                              {score > 0 && (
                                <>
                                  <span className="text-slate-500">•</span>
                                  <span className="text-amber-300">{'★'.repeat(Math.max(1, Math.round(score)))}</span>
                                </>
                              )}
                            </div>
                          </div>
        ...
        {!isLoading && (
          <div className="rounded-xl border border-violet-400/25 bg-slate-900/55 p-3 space-y-3">
                ...
                return (
                  <button
                    ...
                    className={`rounded-lg border px-3 py-1.5 text-xs transition-colors ${
                      isActive
                        ? 'border-violet-300/60 bg-violet-500/30 text-violet-50'
            ...
            <div className="flex items-center justify-between gap-2">
              <div className="text-xs text-slate-400">{resolvedChartConfig.description}</div>
              <button
                type="button"
                onClick={openBooksStatsTab}
                className="h-7 px-2.5 rounded-md border border-cyan-400/40 bg-cyan-500/15 text-cyan-100 text-[11px] font-medium hover:bg-cyan-500/25"
              >
        ...
        {isLoading ? (
          <div className="text-sm text-slate-300">Chargement des statistiques livres...</div>
        ) : (
          <div className="grid grid-cols-1 xl:grid-cols-3 gap-4">
            <div className="xl:col-span-2 grid grid-cols-2 md:grid-cols-3 gap-3">
              <div className="rounded-xl border border-violet-500/30 bg-slate-900/60 p-3">
                <div className="text-xs text-slate-400">Bibliothèque</div>
                <div className="text-xl font-bold text-white">{stats.totalBooks}</div>
                <div className="text-[11px] text-slate-500">en cours {stats.inProgress} • terminés {stats.completed}</div>
              </div>
              <div className="rounded-xl border border-indigo-500/30 bg-slate-900/60 p-3">
                <div className="text-xs text-slate-400 flex items-center gap-1"><BookOpen className="w-3 h-3" /> Sessions</div>
                <div className="text-xl font-bold text-white">{stats.sessionsCount}</div>
                <div className="text-[11px] text-slate-500">pages/session {stats.avgPagesSession}</div>
              </div>
              <div className="rounded-xl border border-blue-500/30 bg-slate-900/60 p-3">
                <div className="text-xs text-slate-400 flex items-center gap-1"><Clock3 className="w-3 h-3" /> Temps total</div>
                <div className="text-xl font-bold text-white">{stats.totalMinutes} min</div>
                <div className="text-[11px] text-slate-500">moy/session {stats.avgMinutesSession} min</div>
              </div>
              <div className="rounded-xl border border-fuchsia-500/30 bg-slate-900/60 p-3">
                <div className="text-xs text-slate-400">Pages lues</div>
                <div className="text-xl font-bold text-white">{stats.totalPagesRead}</div>
                <div className="text-[11px] text-slate-500">30j: {stats.pagesLast30}</div>
              </div>
              <div className="rounded-xl border border-emerald-500/30 bg-slate-900/60 p-3">
                <div className="text-xs text-slate-400">À lire / pause / abandonnés</div>
                <div className="text-xl font-bold text-white">{stats.toRead} / {stats.paused} / {stats.abandoned}</div>
                <div className="text-[11px] text-slate-500">répartition backlog</div>
              </div>
              <div className="rounded-xl border border-amber-500/30 bg-slate-900/60 p-3">
                <div className="text-xs text-slate-400 flex items-center gap-1"><TrendingUp className="w-3 h-3" /> Dernière session</div>
                <div className="text-sm font-bold text-white">
                  {stats.lastSessionDate
                    ? stats.lastSessionDate.toLocaleDateString('fr-FR')
                    : 'Aucune'}
                </div>
                <div className="text-[11px] text-slate-500">activité récente</div>
              </div>
              <div className="rounded-xl border border-cyan-500/30 bg-slate-900/60 p-3">
                <div className="text-xs text-slate-400">Vitesse de lecture</div>
                <div className="text-xl font-bold text-white">{stats.averageSpeed.toFixed(1)} p/h</div>
                <div className="text-[11px] text-slate-500">pages par heure</div>
              </div>
              <div className="rounded-xl border border-rose-500/30 bg-slate-900/60 p-3">
                <div className="text-xs text-slate-400">Rythme de lecture</div>
                <div className="text-xl font-bold text-white">{stats.readingFrequency.toFixed(1)} / sem</div>
                <div className="text-[11px] text-slate-500">sessions par semaine</div>
              </div>
              <div className="rounded-xl border border-sky-500/30 bg-slate-900/60 p-3">
                <div className="text-xs text-slate-400">Jours actifs</div>
                <div className="text-xl font-bold text-white">{stats.uniqueDays}</div>
                <div className="text-[11px] text-slate-500">streak actuel {stats.currentStreak} j</div>
              </div>
            </div>

            <div className="rounded-xl border border-violet-400/30 bg-slate-900/65 p-4 space-y-3">
              <div className="flex items-center justify-between gap-2">
                <div className="text-sm font-semibold text-violet-100">Livre focus</div>
                {selectedBook && (
                  <button
                    type="button"
                    onClick={() => {
              ...
              {selectedBook ? (
                <>
                  <div className="rounded-xl border border-slate-700/70 bg-slate-950/55 p-3 flex items-start gap-3">
                    <div className="h-24 w-16 rounded overflow-hidden border border-slate-700/70 bg-slate-900/80 shrink-0">
                    ...
                    <div className="min-w-0">
                      ...
                      <div className="text-xs text-slate-400 line-clamp-1">{selectedBook.author || 'Auteur non renseigné'}</div>
                  ...
                  <div className="grid grid-cols-2 gap-2 text-xs">
                    <div className="rounded-lg bg-slate-950/60 border border-slate-700/70 p-2">
                      <div className="text-slate-400 flex items-center gap-1"><Target className="w-3 h-3" /> Progression</div>
                      <div className="text-white font-semibold">
                        {getReadingProgressPercent(selectedBook) ?? 0}%
                      </div>
                    </div>
                    <div className="rounded-lg bg-slate-950/60 border border-slate-700/70 p-2">
                      <div className="text-slate-400 flex items-center gap-1"><Timer className="w-3 h-3" /> Temps lu</div>
                      <div className="text-white font-semibold">{getTotalReadingTime(selectedBook)} min</div>
                    </div>
                    <div className="rounded-lg bg-slate-950/60 border border-slate-700/70 p-2">
                      <div className="text-slate-400">Pages lues</div>
                    ...
                    <div className="rounded-lg bg-slate-950/60 border border-slate-700/70 p-2 md:col-span-2">
                      <div className="text-slate-400">Moyennes livre</div>
                      <div className="text-white font-semibold">
                        {getAveragePagesPerSession(selectedBook)} pages/session • {getAverageDurationPerSession(selectedBook)} min/session
                      </div>
                    </div>
                    <div className="rounded-lg bg-slate-950/60 border border-slate-700/70 p-2 md:col-span-2">
                      <div className="text-slate-400 flex items-center gap-1"><Star className="w-3 h-3 text-amber-300" /> Meilleure note terminée</div>
