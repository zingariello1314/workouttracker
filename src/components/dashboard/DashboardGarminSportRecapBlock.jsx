...
import DashboardStrengthRecordedModule from './DashboardStrengthRecordedModule';
...
const intensityBadge = (level) => {
  ...
  return { label: 'Repos', cls: 'bg-teal-950/50 text-teal-200/75 border-teal-500/40' };
};
  ...
  return (
    <div className="relative overflow-hidden rounded-3xl border border-[#0F4C5C]/55 bg-black backdrop-blur-2xl shadow-[0_0_60px_rgba(15,76,92,0.35)]">
      <div
        ...
        style={{
          background:
            'radial-gradient(circle at 0% 0%, rgba(45,212,191,0.12), transparent 50%), radial-gradient(circle at 100% 100%, rgba(15,76,92,0.25), transparent 55%)',
          mixBlendMode: 'screen'
        }}
      />
      <div className="relative p-6 md:p-7 lg:p-8 space-y-6">
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
          <div className="flex items-center gap-3">
            <div className="rounded-2xl bg-[#0F4C5C]/25 p-2.5 border border-teal-400/45 shadow-[0_0_24px_rgba(45,212,191,0.25)]">
              <Activity className="w-6 h-6 text-teal-100" />
            </div>
            <div>
              <h3 className="text-xl font-bold text-white tracking-tight">Sport</h3>
          ...
          <div className="flex items-center gap-3">
            <button
              ...
              className="h-8 px-3 rounded-lg border border-teal-400/45 bg-teal-500/10 text-teal-100 text-xs font-medium flex items-center gap-1.5 hover:bg-teal-500/20 disabled:opacity-50 disabled:cursor-not-allowed"
            ...
            <button
              type="button"
              onClick={goPrevDay}
              className="h-8 w-8 rounded-lg border border-teal-500/45 bg-black text-teal-100 flex items-center justify-center hover:bg-teal-950/50"
            ...
            <button
              ...
              onClick={goNextDay}
              disabled={!canGoNext}
              className="h-8 w-8 rounded-lg border border-teal-500/45 bg-black text-teal-100 flex items-center justify-center hover:bg-teal-950/50 disabled:opacity-40 disabled:cursor-not-allowed"
        ...
        {syncStatus?.message && (
          <div className={`text-xs ${syncStatus?.ok ? 'text-teal-300' : 'text-amber-300'}`}>
            {syncStatus.message}
          </div>
        )}
          ...
          <div className="xl:col-span-2 space-y-4">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 items-start">
              <div className="min-w-0 space-y-4">
                ...
                {loadingGarmin ? (
                  <div className="p-4 bg-black border border-teal-500/45 rounded-2xl text-teal-200/75 text-sm">
              ...
              <div className="min-w-0 space-y-4">
                <DashboardStrengthRecordedModule
                  allData={liveWorkoutData}
                  recapPeriod={recapPeriod}
                  getExerciseNameById={getExerciseNameById}
                  refDate={selectedDate}
                  onRecapPeriodChange={setRecapPeriod}
                />
              </div>
            </div>

            {/* Gros bloc corps + stats, placé juste sous les métriques Garmin */}
            <div className="p-4 rounded-2xl bg-black border border-teal-500/45 space-y-3">
              <div className="flex flex-wrap items-center justify-between gap-2">
                <div className="text-sm font-semibold text-white">Corps (zones colorées)</div>
              ...
              <div className="grid grid-cols-1 2xl:grid-cols-2 gap-4 items-start">
                ...
                />
                <div className="rounded-xl border border-teal-800/35 bg-black p-3">
                  <div className="text-xs font-semibold text-teal-100/90 mb-2">
                  ...
                  <div className="max-h-[520px] overflow-auto pr-1 space-y-1.5">
                    {muscleStatsRows.map((row) => (
                      <div
                        ...
                        className="rounded-lg border border-teal-600/40 bg-black px-2.5 py-2"
                      >
                        <div className="text-xs font-semibold text-white">{row.label}</div>
                        <div className="mt-1 text-[11px] text-teal-200/70">
                          {row.statKind === 'time' ? (
                            <>
                              Temps:{' '}
                              <span className="text-teal-200 font-semibold">{row.statValue} min</span>
                            </>
                          ) : (
                            <>
                              Reps:{' '}
                              <span className="text-teal-300 font-semibold">{row.statValue}</span>
                            </>
                          )}
                        </div>
                      </div>
            ...
            <div className="p-4 rounded-xl bg-black border border-teal-700/35">
              <div className="text-sm font-semibold text-white mb-2">Résumé Garmin 7 jours</div>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-sm">
                <div className="text-teal-100/90">Pas moy/j: <span className="text-white font-semibold">{derived.weekSummary.avgSteps.toLocaleString('fr-FR')}</span></div>
                <div className="text-teal-100/90">FC repos moy: <span className="text-white font-semibold">{derived.weekSummary.avgRestingHr} bpm</span></div>
                <div className="text-teal-100/90">Body Battery moy: <span className="text-white font-semibold">{derived.weekSummary.avgBodyBattery}</span></div>
                <div className="text-teal-100/90">Stress moy: <span className="text-white font-semibold">{derived.weekSummary.avgStress}</span></div>
                <div className="text-teal-100/90">Sommeil moy: <span className="text-white font-semibold">{derived.weekSummary.avgSleepHours} h</span></div>
                <div className="text-teal-100/90">Cal. actives moy: <span className="text-white font-semibold">{derived.weekSummary.avgActiveCalories} kcal</span></div>
                <div className="text-teal-100/90">Intensité totale: <span className="text-white font-semibold">{derived.weekSummary.totalIntensityMinutes} min</span></div>
                <div className="text-teal-100/90">Jours avec data: <span className="text-white font-semibold">{derived.weekSummary.daysWithMetrics}/7</span></div>
                <div className="text-teal-100/90">Activités sur 7j: <span className="text-white font-semibold">{derived.weekSummary.totalActivities}</span></div>
              </div>
            </div>

            <div className="p-4 rounded-xl bg-black border border-teal-700/35 space-y-3">
              <div className="text-sm font-semibold text-white">Activités Garmin ({formatDateFr(selectedDateKey)})</div>
              {derived.selectedActivitiesCount === 0 ? (
                <div className="text-sm text-teal-200/70">Aucune activité enregistrée ce jour.</div>
              ) : (
                <>
                  {derived.selectedActivities.swimming.map((act, idx) => {
                    const swim = act.swimmingMetrics || {};
                    return (
                      <div key={`swim-${idx}`} className="rounded-xl bg-teal-950/40 border border-teal-500/40 p-3">
                        <div className="text-teal-200 text-sm font-semibold mb-2">🏊 Natation</div>
                  ...
                  {derived.selectedActivities.jumpRope.map((act, idx) => {
                    ...
                    return (
                      <div key={`jump-${idx}`} className="rounded-xl bg-teal-950/40 border border-teal-500/40 p-3">
                        <div className="text-teal-200 text-sm font-semibold mb-2">🪢 Corde à sauter</div>
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-2 text-xs text-white">
                          <div>Durée: <span className="text-white">{formatDuration(act.duration)}</span></div>
            ...
            {periodStats && (
              <div className="p-4 rounded-xl bg-black border border-teal-700/35 space-y-3">
                <div className="flex items-center justify-between">
                  <div className="text-sm font-semibold text-white">Métriques Garmin (période)</div>
                  <div className="text-xs text-teal-200/70">{periodStats.days} jours</div>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-3">
                  <div className="rounded-xl border border-rose-700/40 bg-rose-900/10 p-3">
                    ...
                    <div className="space-y-2 text-xs">
                      <div className="text-teal-100/90">Repos <span className="text-white font-semibold float-right">{periodStats.heartRate.resting.avg.toFixed(1)} bpm</span></div>
                      <div className="text-teal-300/55">Min: {periodStats.heartRate.resting.min.toFixed(1)} • Max: {periodStats.heartRate.resting.max.toFixed(1)} • {trendLabel(periodStats.heartRate.resting.trend)}</div>
                      <div className="border-t border-teal-800/35 pt-2 text-teal-100/90">Moyenne <span className="text-white font-semibold float-right">{periodStats.heartRate.avg.avg.toFixed(1)} bpm</span></div>
                      <div className="text-teal-300/55">Min: {periodStats.heartRate.avg.min.toFixed(1)} • Max: {periodStats.heartRate.avg.max.toFixed(1)} • {trendLabel(periodStats.heartRate.avg.trend)}</div>
                      <div className="border-t border-teal-800/35 pt-2 text-teal-100/90">Maximum <span className="text-white font-semibold float-right">{periodStats.heartRate.max.avg.toFixed(1)} bpm</span></div>
                      <div className="text-teal-300/55">Min: {periodStats.heartRate.max.min.toFixed(1)} • Max: {periodStats.heartRate.max.max.toFixed(1)} • {trendLabel(periodStats.heartRate.max.trend)}</div>
                    </div>
                  </div>
                  ...
                  <div className="rounded-xl border border-indigo-700/40 bg-indigo-900/10 p-3">
                    ...
                    <div className="text-xs text-teal-100/90">Moyenne <span className="text-white font-semibold float-right">{Math.round(periodStats.steps.avg).toLocaleString('fr-FR')}</span></div>
                    <div className="text-xs text-teal-300/55 mt-1">Min: {Math.round(periodStats.steps.min).toLocaleString('fr-FR')} • Max: {Math.round(periodStats.steps.max).toLocaleString('fr-FR')} • {trendLabel(periodStats.steps.trend)}</div>
                    <div className="border-t border-teal-800/35 pt-2 mt-2 text-xs text-teal-100/90">Total période <span className="text-white font-semibold float-right">{Math.round(periodStats.steps.total).toLocaleString('fr-FR')}</span></div>
                  </div>
                  ...
                  <div className="rounded-xl border border-pink-700/40 bg-pink-900/10 p-3">
                    ...
                    <div className="text-xs text-teal-100/90">Quotidienne <span className="text-white font-semibold float-right">{periodStats.distance.avg.toFixed(1)} km</span></div>
                    <div className="text-xs text-teal-300/55 mt-1">Min: {periodStats.distance.min.toFixed(1)} • Max: {periodStats.distance.max.toFixed(1)} • {trendLabel(periodStats.distance.trend)}</div>
                    <div className="border-t border-teal-800/35 pt-2 mt-2 text-xs text-teal-100/90">Total période <span className="text-white font-semibold float-right">{periodStats.distance.total.toFixed(2)} km</span></div>
                  </div>
                  ...
                  <div className="rounded-xl border border-amber-700/40 bg-amber-900/10 p-3">
                    ...
                    <div className="text-xs text-teal-100/90">Total <span className="text-white font-semibold float-right">{periodStats.calories.total.avg.toFixed(1)} kcal</span></div>
                    <div className="text-xs text-teal-300/55 mt-1">Min: {periodStats.calories.total.min.toFixed(1)} • Max: {periodStats.calories.total.max.toFixed(1)} • {trendLabel(periodStats.calories.total.trend)}</div>
                    <div className="border-t border-teal-800/35 pt-2 mt-2 text-xs text-teal-100/90">Active <span className="text-white font-semibold float-right">{periodStats.calories.active.avg.toFixed(1)} kcal</span></div>
                    <div className="text-xs text-teal-300/55 mt-1">Min: {periodStats.calories.active.min.toFixed(1)} • Max: {periodStats.calories.active.max.toFixed(1)} • {trendLabel(periodStats.calories.active.trend)}</div>
                    <div className="border-t border-teal-800/35 pt-2 mt-2 text-xs text-teal-100/90">Total période <span className="text-white font-semibold float-right">{Math.round(periodStats.calories.total.total).toLocaleString('fr-FR')} kcal</span></div>
                  </div>
                  ...
                  <div className="rounded-xl border border-lime-700/40 bg-lime-900/10 p-3">
                    ...
                    <div className="text-xs text-teal-100/90">Niveau <span className="text-white font-semibold float-right">{periodStats.bodyBattery.avg.toFixed(1)}/100</span></div>
                    <div className="text-xs text-teal-300/55 mt-1">Min: {periodStats.bodyBattery.min.toFixed(1)}/100 • Max: {periodStats.bodyBattery.max.toFixed(1)}/100 • {trendLabel(periodStats.bodyBattery.trend)}</div>
                  </div>
                  ...
                  <div className="rounded-xl border border-fuchsia-700/40 bg-fuchsia-900/10 p-3">
                    ...
                    <div className="text-xs text-teal-100/90">Niveau <span className="text-white font-semibold float-right">{periodStats.stress.avg.toFixed(1)}/100</span></div>
                    <div className="text-xs text-teal-300/55 mt-1">Min: {periodStats.stress.min.toFixed(1)}/100 • Max: {periodStats.stress.max.toFixed(1)}/100 • {trendLabel(periodStats.stress.trend)}</div>
                  </div>
                  ...
                  <div className="rounded-xl border border-sky-700/40 bg-sky-900/10 p-3 md:col-span-2 xl:col-span-1">
                    ...
                    <div className="text-xs text-teal-100/90">Durée <span className="text-white font-semibold float-right">{periodStats.sleep.avg.toFixed(1)} h</span></div>
                    <div className="text-xs text-teal-300/55 mt-1">Min: {periodStats.sleep.min.toFixed(1)} h • Max: {periodStats.sleep.max.toFixed(1)} h • {trendLabel(periodStats.sleep.trend)}</div>
                    <div className="border-t border-teal-800/35 pt-2 mt-2 text-xs text-teal-100/90">Total période <span className="text-white font-semibold float-right">{periodStats.sleep.total.toFixed(1)} h</span></div>
                  </div>
                </div>
              </div>
            )}
            ...
            <div className="p-4 rounded-2xl bg-black border border-teal-500/45">
              <div className="flex items-center justify-between gap-2 mb-2">
                ...
                <button
                  ...
                  className="h-7 px-2.5 rounded-md border border-teal-500/45 bg-teal-500/10 text-teal-100 text-[11px] font-medium hover:bg-teal-500/20"
                >
              ...
              {selectedWorkout?.exercices?.length ? (
                <>
                  <div className="text-sm text-teal-100/90 mb-2">
                  ...
                  <div className="space-y-1 text-sm">
                    <div className="text-teal-100/90">Programme: <span className="text-white font-semibold">{todayProgress.planned} exos</span></div>
                    <div className="text-teal-100/90">Exceptionnels: <span className="text-white font-semibold">{todayProgress.additional}</span></div>
                    <div className="text-teal-100/90">Réalisés: <span className="text-white font-semibold">{todayProgress.done}</span></div>
                    <div className="text-teal-100/90">Avancement: <span className="text-teal-300 font-semibold">{todayProgress.completionPct}%</span></div>
                  </div>
                  <div className="mt-3 max-h-56 overflow-auto space-y-2 pr-1">
                    {programExercises.map((exercise) => {
                      ...
                      return (
                        <div
                          key={exercise.id}
                          className="flex items-start gap-2 p-2 rounded-lg border border-teal-700/35 bg-black"
                        >
                          <input
                            type="checkbox"
                            checked={checked}
                          ...
                          <div className="text-sm min-w-0">
                            <div className={`font-medium ${checked ? 'text-teal-300 line-through' : 'text-white'}`}>
                              {exercise.name || `Exercice ${exercise.id}`}
                            </div>
                            <div className="text-xs text-teal-200/70">{exercise.series || 'Séries non définies'}</div>
                          </div>
                            ...
                            <input
                              ...
                              className={`w-20 rounded-md px-2 py-1 text-xs text-center border ${
                                checked
                                  ...
                                  : 'bg-black border-teal-600/45 text-white'
                              }`}
                            />
                            <span className="text-[11px] text-teal-200/70 min-w-[26px]">{inputUnitLabel}</span>
                          </div>
                        </div>
                      );
                    })}
                  </div>
                  <div className="mt-4 border-t border-teal-600/40 pt-3">
                    <div className="text-xs font-semibold text-teal-100/90 mb-2">Étirements du jour</div>
                    {stretchEntries.length ? (
                        ...
                        {stretchEntries.map(({ moment, description }) => {
                          ...
                          return (
                            ...
                            >
                              ...
                              />
                              <div className="min-w-0">
                                <div className={`text-sm font-medium ${checked ? 'text-teal-300 line-through' : 'text-white'}`}>
                                  {label}
                                </div>
                                <div className="text-xs text-teal-200/70">{description}</div>
                              </div>
                            </div>
                          );
                        })}
                      ...
                      <div className="text-xs text-teal-300/55">Aucun étirement défini pour cette séance.</div>
                    )}
                  </div>
                </>
              ) : (
                <div className="text-sm text-teal-200/70">Aucune séance planifiée ce jour (repos ou programme vide).</div>
              )}
            </div>
