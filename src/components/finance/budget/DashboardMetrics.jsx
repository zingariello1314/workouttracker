...
const DashboardMetrics = memo(() => {
  ...
  const { calculateMetrics } = useBudget();
    ...
    <div className="dashboard-metrics grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
      ...
      <div className="bg-slate-800/50 border border-slate-700/50 rounded-lg p-6">
        ...
        <div className="text-2xl font-bold text-white mb-1">
          {formatCurrency(metrics.revenus)}
        </div>
      ...
      <div className="bg-slate-800/50 border border-slate-700/50 rounded-lg p-6">
        ...
        <div className="text-2xl font-bold text-white mb-1">
          {formatCurrency(metrics.depenses)}
        </div>
        <div className="text-sm text-slate-400">
          {metrics.pourcentUtilise.toFixed(1)}% du budget
        </div>
      </div>
      ...
      <div className="bg-slate-800/50 border border-slate-700/50 rounded-lg p-6">
        ...
        <div className="text-2xl font-bold text-white mb-1">
          {formatCurrency(metrics.epargne)}
        </div>
        <div className="text-sm text-slate-400">Actuelle</div>
      </div>

      {/* Restant */}
      <div className={`bg-slate-800/50 border rounded-lg p-6 ${getStatutColor(metrics.statut)}`}>
        <div className="flex items-center justify-between mb-2">
          <span className="text-2xl">📊</span>
          <span className="text-xs uppercase">Restant</span>
        </div>
        <div className="text-2xl font-bold mb-1">
          {formatCurrency(metrics.restant)}
        </div>
        <div className="text-sm font-medium">
          {getStatutLabel(metrics.statut)}
        </div>
      </div>
    </div>
  );
