  ...
  return (
    <div className="rounded-xl border-2 border-[#0F4C5C]/70 bg-black p-4 shadow-md shadow-black/40">
      <div className="flex justify-between items-start mb-3">
        <div>
          <h4 className="text-teal-100 font-semibold">🏊 Natation - {activity.date} {activity.time}</h4>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-3 text-sm">
        ...
        <div className="bg-black border border-[#0F4C5C]/40 rounded-lg p-2">
          <div className="text-teal-100/55 text-xs">Longueurs</div>
          <div className="text-teal-100 font-semibold">{swimming.laps || activity.laps || 0}</div>
        </div>
        <div className="bg-black border border-[#0F4C5C]/40 rounded-lg p-2">
        ...
        {swimming.strokeCount > 0 && (
          <div className="bg-black border border-[#0F4C5C]/40 rounded-lg p-2">
            <div className="text-teal-100/55 text-xs">Mouvements</div>
            <div className="text-teal-100 font-semibold">{swimming.strokeCount}</div>
          </div>
        )}
        {swimming.avgStrokeRate > 0 && (
          <div className="bg-black border border-[#0F4C5C]/40 rounded-lg p-2">
            <div className="text-teal-100/55 text-xs">Fréquence moyenne</div>
            <div className="text-teal-100 font-semibold">{swimming.avgStrokeRate.toFixed(1)} /min</div>
          </div>
        )}
        {swimming.avgMovementsPerLap > 0 && (
          <div className="bg-black border border-[#0F4C5C]/40 rounded-lg p-2">
            <div className="text-teal-100/55 text-xs">Mouvements/longueur</div>
            <div className="text-teal-100 font-semibold">{swimming.avgMovementsPerLap.toFixed(1)}</div>
          </div>
        )}
        ...
        {swimming.avgSwolf > 0 && (
          <div className="bg-black border border-[#0F4C5C]/40 rounded-lg p-2">
            <div className="text-teal-100/55 text-xs">SWOLF moyen</div>
            <div className="text-teal-100 font-semibold">{swimming.avgSwolf.toFixed(1)}</div>
          </div>
        )}
        ...
        {swimming.avgSpeedMovement > 0 && (
          <div className="bg-black border border-[#0F4C5C]/40 rounded-lg p-2">
            <div className="text-teal-100/55 text-xs">Vitesse déplacement</div>
            <div className="text-teal-100 font-semibold">{formatSpeed(swimming.avgSpeedMovement)}</div>
          </div>
        )}
        {swimming.avgPaceMovement > 0 && (
          <div className="bg-black border border-[#0F4C5C]/40 rounded-lg p-2">
            <div className="text-teal-100/55 text-xs">Allure déplacement</div>
            <div className="text-teal-100 font-semibold">{formatPace(swimming.avgPaceMovement)} /100m</div>
          </div>
        )}
        {swimming.maxSpeed > 0 && (
          <div className="bg-black border border-[#0F4C5C]/40 rounded-lg p-2">
            <div className="text-teal-100/55 text-xs">Vitesse max</div>
            <div className="text-teal-100 font-semibold">{formatSpeed(swimming.maxSpeed)}</div>
          </div>
        )}
        {swimming.poolLength && (
          <div className="bg-black border border-[#0F4C5C]/40 rounded-lg p-2">
            <div className="text-teal-100/55 text-xs">Longueur piscine</div>
            <div className="text-teal-100 font-semibold">{swimming.poolLength}m</div>
          </div>
        )}
        ...
        {swimming.avgPace > 0 && (
          <div className="bg-black border border-[#0F4C5C]/40 rounded-lg p-2">
            ...
            <div className="text-teal-100 font-semibold">{formatPace(swimming.avgPace)} /100m</div>
          </div>
        )}
        {swimming.bestPace > 0 && (
          <div className="bg-black border border-[#0F4C5C]/40 rounded-lg p-2">
            <div className="text-teal-100/55 text-xs">Meilleure allure</div>
            <div className="text-teal-100 font-semibold">{formatPace(swimming.bestPace)} /100m</div>
          </div>
        )}
        {swimming.avgSpeed > 0 && (
          <div className="bg-black border border-[#0F4C5C]/40 rounded-lg p-2">
            <div className="text-teal-100/55 text-xs">Vitesse moyenne</div>
            <div className="text-teal-100 font-semibold">{formatSpeed(swimming.avgSpeed)}</div>
          </div>
        )}
        ...
        {time.activeTime > 0 && (
          <div className="bg-black border border-[#0F4C5C]/40 rounded-lg p-2">
            <div className="text-teal-100/55 text-xs">Temps actif</div>
            <div className="text-teal-100 font-semibold">{formatDuration(time.activeTime)}</div>
          </div>
        )}
        {time.totalTime > 0 && (
          <div className="bg-black border border-[#0F4C5C]/40 rounded-lg p-2">
            <div className="text-teal-100/55 text-xs">Temps total</div>
            <div className="text-teal-100 font-semibold">{formatDuration(time.totalTime)}</div>
          </div>
        )}
        ...
        {swimming.laps && Array.isArray(swimming.laps) && swimming.laps.length > 0 ? (
          <div className="bg-black border border-[#0F4C5C]/40 rounded-lg p-2 md:col-span-3 text-xs">
            <div className="text-teal-100/55 mb-2 font-semibold">Détail des longueurs</div>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
              {swimming.laps.map((lap, idx) => (
                <div key={idx} className="bg-teal-950/20 border border-[#0F4C5C]/30 px-2 py-1 rounded-lg">
                  <div className="text-teal-100/45 text-xs">#{lap.lapNumber}</div>
                  {lap.time > 0 && <div className="text-teal-100 text-sm">{formatDuration(lap.time)}</div>}
                  {lap.distance > 0 && <div className="text-teal-100/55 text-xs">{lap.distance}m</div>}
                  {lap.strokeCount > 0 && <div className="text-teal-100/55 text-xs">{lap.strokeCount} mouvements</div>}
                </div>
              ))}
            </div>
          </div>
        ...
        {activity.elevation && (
          <div className="bg-black border border-[#0F4C5C]/40 rounded-lg p-2 md:col-span-3 text-xs">
            <div className="text-teal-100/55 mb-1">Élévation</div>
            <div className="text-sky-300/85">
