...
export default function GarminDashboard() {
  ...
  const renderMetricCard = (label, value, _colorType, compareValue = null) => {
    ...
    const diff =
      numericCompareValue !== null && typeof numericCompareValue === 'number' && !isNaN(numericCompareValue)
        ? numericValue - numericCompareValue
        : null;
    const diffDisplay = diff !== null && typeof diff === 'number' && !isNaN(diff) ? (
      <div className={`text-xs mt-1 ${diff > 0 ? 'text-emerald-400' : diff < 0 ? 'text-red-400' : 'text-teal-100/45'}`}>
    ...
    return (
      <div className="rounded-xl border-2 border-[#0F4C5C]/60 bg-black p-3 shadow-md shadow-black/25">
        <div className="text-teal-100/60 text-xs mb-1">{label}</div>
        <div className="text-teal-100 text-xl font-bold">{displayText}</div>
        {diffDisplay}
      </div>
    );
  };
  ...
  return (
    <div className="mt-6">
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-teal-100 font-semibold">📊 Dashboard - {displayDate}</h3>
        {comparisonMode && compareDate && (
          <div className="text-sm text-sky-300/80">
            Comparaison avec <span className="font-semibold text-teal-100">{compareDate}</span>
          </div>
        )}
      </div>
      ...
      {comparisonMode && compareDate && (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
          ...
          <div>
            <h4 className="text-teal-100/70 font-medium mb-3 text-sm">{displayDate}</h4>
          ...
          <div>
            <h4 className="text-teal-100/70 font-medium mb-3 text-sm">{compareDate}</h4>
            <div className="grid grid-cols-2 gap-3">
      ...
      {!comparisonMode && (
        ...
        {/* Carte Pas */}
        <div className="rounded-xl border-2 border-[#0F4C5C]/60 bg-black p-4 shadow-md shadow-black/25">
          <div className="text-teal-100/65 text-xs mb-1">Pas</div>
          <div className="text-teal-100 text-2xl font-bold">{extractNumeric(d.steps)}</div>
        <div className="text-sky-300/75 text-xs mt-2">
        ...
        {/* Carte Calories */}
        <div className="rounded-xl border-2 border-[#0F4C5C]/60 bg-black p-4 shadow-md shadow-black/25">
          <div className="text-teal-100/65 text-xs mb-1">Calories</div>
          <div className="text-teal-100 text-2xl font-bold">{extractNumeric(calories.total)}</div>
          <div className="text-sky-300/75 text-xs mt-2">
        ...
        {/* Carte FC */}
        <div className="rounded-xl border-2 border-[#0F4C5C]/60 bg-black p-4 shadow-md shadow-black/25">
          <div className="text-teal-100/65 text-xs mb-1">FC Repos</div>
          <div className="text-teal-100 text-2xl font-bold">{extractNumeric(hr.resting)}</div>
          <div className="text-sky-300/75 text-xs mt-2">
        ...
        {d.sleep && d.sleep.duration > 0 && (
          <div className="rounded-xl border-2 border-[#0F4C5C]/60 bg-black p-4 shadow-md shadow-black/25">
            <div className="text-teal-100/65 text-xs mb-1">Sommeil</div>
            <div className="text-teal-100 text-2xl font-bold">
              {formatSleepDuration(d.sleep.duration)}
            </div>
            {extractNumeric(d.sleep.quality) > 0 && (
              <div className="text-sky-300/75 text-xs mt-2">Qualité: {extractNumeric(d.sleep.quality)}/100</div>
            )}
          </div>
        )}
        ...
        {(() => {
          ...
          return bodyBatteryValue !== null && (
            <div className="rounded-xl border-2 border-[#0F4C5C]/60 bg-black p-4 shadow-md shadow-black/25">
              <div className="text-teal-100/65 text-xs mb-1">Body Battery</div>
              <div className="text-teal-100 text-2xl font-bold">{bodyBatteryValue}/100</div>
              <div className="mt-2 w-full rounded-full h-2 border border-[#0F4C5C]/35 bg-black">
                <div
                  className={`h-2 rounded-full ${
                    bodyBatteryValue >= 70 ? 'bg-emerald-400' :
                    bodyBatteryValue >= 50 ? 'bg-amber-400' :
        ...
        {(() => {
          ...
          return stressValue !== null && (
            <div className="rounded-xl border-2 border-[#0F4C5C]/60 bg-black p-4 shadow-md shadow-black/25">
              <div className="text-teal-100/65 text-xs mb-1">Stress</div>
              <div className="text-teal-100 text-2xl font-bold">{stressValue}</div>
              <div className="mt-2 w-full rounded-full h-2 border border-[#0F4C5C]/35 bg-black">
                <div
                  className={`h-2 rounded-full ${
                    stressValue <= 25 ? 'bg-emerald-400' :
                    stressValue <= 50 ? 'bg-amber-400' :
        ...
        {d.spo2 !== undefined && d.spo2 !== null && (() => {
          ...
          return spo2Value > 0 && (
          <div className="rounded-xl border-2 border-[#0F4C5C]/60 bg-black p-4 shadow-md shadow-black/25">
            <div className="text-teal-100/65 text-xs mb-1">SpO2</div>
              <div className="text-teal-100 text-2xl font-bold">{spo2Value}%</div>
            <div className={`text-xs mt-2 ${
                spo2Value >= 95 ? 'text-emerald-400' :
                spo2Value >= 90 ? 'text-amber-300' : 'text-red-400'
            }`}>
        ...
        {d.intensityMinutes && (() => {
          ...
          <div className="rounded-xl border-2 border-[#0F4C5C]/60 bg-black p-4 shadow-md shadow-black/25">
            <div className="text-teal-100/65 text-xs mb-1">Intensité</div>
              <div className="text-teal-100 text-2xl font-bold">{total} min</div>
            <div className="text-sky-300/75 text-xs mt-2">
