  ...
  const { intro, cards } = useMemo(() => {
    ...
    (blocks || []).forEach((block) => {
      ...
      if (!current) {
        ...
        if (block.type === 'figure') intro.push({ kind: 'figure', block });
        return;
      }
      if (block.type === 'p') current.body.push({ kind: 'p', text: block.text });
      if (block.type === 'figure') current.body.push({ kind: 'figure', block });
          ...
          {intro.map((t, i) =>
            typeof t === 'string' ? (
              ...
              <AnatomyContentFigure
                ...
                src={t.block.src}
                alt={t.block.alt}
                caption={t.block.caption}
                layout={t.block.layout}
              />
            )
          )}
        </div>
