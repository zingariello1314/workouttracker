...
function ArticleReaderModal({ selected, isAdmin, onClose, onDelete, t }) {
      ...
      <article className="max-h-[85vh] w-full max-w-2xl overflow-y-auto rounded-2xl border border-violet-500/30 bg-[#0a0612] p-6 shadow-2xl">
        ...
        {selected.externalUrl ? (
          <a
            ...
            className="mt-2 inline-flex items-center gap-1 text-sm text-violet-300 hover:underline"
          >
...
function ArticleFeedCard({ art, onOpen, categoriesById }) {
  const cats = (art.categoryIds || [])
  ...
  return (
    ...
    >
      <p className="text-[10px] uppercase tracking-wide text-violet-400/70">
        {formatKnowledgeDate(art.createdAt)}
      </p>
      <h3 className="mt-1 text-base font-semibold text-white">{art.title}</h3>
      {art.excerpt ? <p className="mt-2 line-clamp-3 text-sm text-slate-400">{art.excerpt}</p> : null}
      {cats.length > 0 ? (
...
function ArticleListItem({ art, onOpen }) {
  return (
    <button
      type="button"
      onClick={() => onOpen(art.id)}
    ...
    >
      <h3 className="font-semibold text-slate-100">{art.title}</h3>
      {art.excerpt ? <p className="mt-1 line-clamp-2 text-xs text-slate-500">{art.excerpt}</p> : null}
    </button>
  );
}

export default function KnowledgeArticlesView({ isAdmin, categories, hiddenCategoryIds, onCatalogReload }) {
  const t = useTranslation();
  const [section, setSection] = useState(() =>
    readStoredSection(KNOWLEDGE_ARTICLE_SECTION_LS, KNOWLEDGE_TEXT_SECTIONS, KNOWLEDGE_SECTION.FEED)
  );
  ...
  const [form, setForm] = useState({ title: '', body: '', externalUrl: '', categoryIds: [] });
  ...
  useEffect(() => {
    try {
      localStorage.setItem(KNOWLEDGE_ARTICLE_SECTION_LS, section);
    } catch {
      /* ignore */
    }
  }, [section]);
  ...
  const loadLibrary = useCallback(async () => {
    let groups = await fetchKnowledgeLibraryGrouped('articles', {
      search: debouncedSearch || undefined,
      hiddenCategoryIds
    });
    if (categoryId) {
  ...
  const openArticle = async (id) => {
    const full = await fetchKnowledgeArticle(id);
    setSelected(full);
  };
  ...
  const handleCreate = async () => {
    ...
    setShowForm(false);
    setForm({ title: '', body: '', externalUrl: '', categoryIds: [] });
  ...
  return (
    <div className="space-y-4">
      ...
      {loading ? (
        ...
        items.length === 0 ? (
          <KnowledgeEmptyState title={t('knowledge.emptyArticles')} hint={t('knowledge.emptyArticlesHint')} />
        ) : (
          <div className="space-y-3">
            {items.map((art) => (
              <ArticleFeedCard key={art.id} art={art} onOpen={openArticle} categoriesById={categoriesById} />
            ))}
          ...
          {libraryGroups.map((group) => (
            <section key={group.categoryId ?? '__none'}>
              ...
              <ul className="space-y-2">
                {group.items.map((art) => (
                  <li key={art.id}>
                    <ArticleListItem art={art} onOpen={openArticle} />
                  </li>
                ))}
              </ul>
            </section>
      ...
      {showForm && isAdmin ? (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 p-4 backdrop-blur-sm">
          <div className="w-full max-w-lg rounded-2xl border border-violet-500/30 bg-[#0a0612] p-5 shadow-xl">
            <h3 className="mb-3 font-semibold text-white">{t('knowledge.newArticle')}</h3>
            <div className="space-y-2">
              ...
              <input
                ...
                placeholder={t('knowledge.articleUrl')}
                className="w-full rounded-xl border border-violet-500/25 bg-black/60 px-3 py-2 text-sm text-white"
              />
              <textarea
                value={form.body}
                ...
                rows={8}
                placeholder={t('knowledge.articleBody')}
                className="w-full rounded-xl border border-violet-500/25 bg-black/60 px-3 py-2 text-sm text-white"
              />
